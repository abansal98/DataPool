...
app.post('/data', function (req, res) {
    var rc = [];
    var args = req.body;
    for (var i = 0; i < tempDataSample.length; i++) {
        if (tempDataSample[i].processCode == args.processCode.toUpperCase()) {
            rc.push(tempDataSample[i]);
        }
    }
    if (req.session.user != undefined)
        // res.send(docs);
        res.render('dataSearch', { data: rc, env: env, user: req.session.user, show: "noshow" });
    else
        // res.send(docs);
        res.render('dataSearch', { data: rc, env: env, user: "hide", show: "noshow" });
    // res.render('dataSearch', { data: rc });
})
